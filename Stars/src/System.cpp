...
System::System(bool sun)
{
	...
	if (sun)
	{
		...
		orbits[0]->planet->speed = 4.14f;
		orbits[0]->planet->dist = padding + 6.f;
		orbits[0]->planet->origin = 360 * dist(e);

		// venera
		rotDir = 0.5f * (dist(e) * 0.5f + 1.f) * glm::normalize(glm::vec3{ dist(e) * 2 - 1, dist(e) * 2 - 1, dist(e) * 2 - 1 });
		orbits.emplace_back(new _orbit_);
		orbits[1]->planet = new Planet(0.121f, 0, 800, { 0.6, 0.4, 0.2 }, rotDir, {}, 1);
		orbits[1]->planet->speed = 1.62f;
		orbits[1]->planet->dist = padding + 8.f;
		orbits[1]->planet->origin = 360 * dist(e);
		...
		orbits[2]->planet->speed = 1;
		orbits[2]->planet->dist = padding + 10.f;
		...
		orbits[2]->planet->origin = 360 * dist(e);
		...
		orbits[3]->planet->speed = 0.53f;
		orbits[3]->planet->dist = padding + 12.f;
		orbits[3]->planet->min_height = 0.48f;
		orbits[3]->planet->max_height = 1.f;
		orbits[3]->planet->origin = 360 * dist(e);

		// jupiteris
		rotDir = 0.3f * (dist(e) * 0.5f + 1.f) * glm::normalize(glm::vec3{ dist(e) * 2 - 1, dist(e) * 2 - 1, dist(e) * 2 - 1 });
		orbits.emplace_back(new _orbit_);
		orbits[4]->planet = new Planet(1.43f, 7, 8000, { 0.9, 0.6, 0.3 }, rotDir, {}, 2);
		orbits[4]->planet->speed = 0.083f;
		orbits[4]->planet->dist = padding + 15.f;
		orbits[4]->planet->origin = 360 * dist(e);

		// saturnas
		rotDir = 0.3f * (dist(e) * 0.5f + 1.f) * glm::normalize(glm::vec3{ dist(e) * 2 - 1, dist(e) * 2 - 1, dist(e) * 2 - 1 });
		orbits.emplace_back(new _orbit_);
		orbits[5]->planet = new Planet(1.20f, 0, 600, { 0.8, 0.8, 0.4 }, rotDir, rotDir, 2, { 0.6, 0.6, 0.2 }, true);
		orbits[5]->planet->speed = 0.03f;
		orbits[5]->planet->dist = padding + 23.f;
		orbits[5]->planet->origin = 360 * dist(e);
		...
		orbits[6]->planet->speed = 0.012f;
		orbits[6]->planet->dist = padding + 28.f;
		orbits[6]->planet->origin = 360 * dist(e);

		// neptunas
		rotDir = 0.25f * (dist(e) * 0.5f + 1.f) * glm::normalize(glm::vec3{ dist(e) * 2 - 1, dist(e) * 2 - 1, dist(e) * 2 - 1 });
		orbits.emplace_back(new _orbit_);
		orbits[7]->planet = new Planet(0.50f, 0, 500, { 0.4, 0.6, 0.8 }, rotDir, {}, 2);
		orbits[7]->planet->speed = 0.006f;
		orbits[7]->planet->dist = padding + 30.f;
		orbits[7]->planet->origin = 360 * dist(e);
	}
	else
	{
		// STARS
			...
			if (*ImGuiWindow::Variable_int("OrbitSelectedID") >= 1)
			{
				...
				if (orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->planet != nullptr)
				{
						...
						*ImGuiWindow::Variable_int("PlanetSelectedID") = 0;
						if (*ImGuiWindow::Variable_bool("CreatorFocusPlanet"))
						{
							...
							*ImGuiWindow::Variable_int("SmoothMove") = Global::maxFrames;
							...
							player->orbitDist = output->size * 2;
							player->orbitSpeed = player->orbitDist / 50;
							player->orbitMin = output->size * 1.1f;
						}
					}
					...
					if (ImGui::Button("Add Planet"))
					{
						...
						orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->planet->isPlanet = true;
						orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->planet->dist = 25.f;
					}
				// moons
				...
				if (orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->planet != nullptr)
					if (ImGui::Button("Add Moon"))
					{
						...
						orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->moons.back()->isMoon = true;
						orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->moons.back()->size = 0.03f;
						orbits[*ImGuiWindow::Variable_int("OrbitSelectedID") - 1]->moons.back()->dist = 0.2f;
					}
			}
		...
		if (ImGui::BeginChild("Settings", { 368, 300 }, true, ImGuiWindowFlags_NoScrollbar))
		{
			...
			if (*ImGuiWindow::Variable_int("OrbitSelectedID") >= 0 && *ImGuiWindow::Variable_int("PlanetSelectedID") >= 0)
			{
				...
				if (ImGui::BeginChild("ShortSettings", { 352, 170 }, true, ImGuiWindowFlags_NoScrollbar))
				{
					...
					if (ImGui::DragFloat("Size", &it->size, 0.01f));
					if (ImGui::DragInt("Detail", &it->detail, 0.01f, 0, 8));
					if (ImGui::DragFloat("Noisy", &it->mult, 1.f));
					if (ImGui::DragFloat("Max Height", &it->max_height, 0.01f));
					if (ImGui::DragFloat("Min Height", &it->min_height, 0.01f));
					if (ImGui::DragFloat("Origin", &it->origin));
					if (ImGui::DragFloat("Distance", &it->dist, 0.01f));
					ImGui::PopItemWidth();
					ImGui::NextColumn();
					ImGui::PushItemWidth(80);
					if (ImGui::Checkbox("Sun", &it->isSun));
					if (ImGui::Checkbox("Planet", &it->isPlanet));
					if (ImGui::Checkbox("Gas", &it->isGas));
					if (ImGui::Checkbox("Moon", &it->isMoon));
					if (ImGui::Checkbox("Ring", &it->hasRing));
					if (ImGui::DragFloat("Ring Size", &it->atmoSize, 0.01f));
					if (ImGui::DragFloat("Speed", &it->speed, 0.01f));
					ImGui::PopItemWidth();
					ImGui::Columns();

					ImGui::EndChild();
				}
				if (ImGui::DragFloat4("Color", glm::value_ptr(it->color), 0.01f, 0.001f));
				if (ImGui::DragFloat3("Rotation", glm::value_ptr(it->planetRot), 0.01f, 0.001f));
				if (ImGui::DragFloat4("Ring Color", glm::value_ptr(it->colorRing), 0.01f, 0.001f));
				if (ImGui::DragFloat3("Ring Rotation", glm::value_ptr(it->atmoRot), 0.01f, 0.001f));
			}
			ImGui::EndChild();
		}
		ImGui::End();
